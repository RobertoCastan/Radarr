...
Radarr does not run aMule itself and does not shell out to `amulecmd`; it talks to
...
services:
  amule:
    ...
    ports:
      - "4712:4712"      # External Connections, required by Radarr
...
and port `4712` reachable from Radarr.

## Path mapping

Use a dedicated aMule category such as `radarr`. Radarr filters the aMule queue
...
The aMule category/Incoming path and the path Radarr can read must resolve to
the same files. Either mount the same host path at the same container path in
both containers, or configure Radarr remote path mapping for the aMule host.

If Radarr needs to create the category automatically, aMule must report a
