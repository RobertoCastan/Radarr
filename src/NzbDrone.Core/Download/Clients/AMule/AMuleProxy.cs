...
namespace NzbDrone.Core.Download.Clients.AMule
{
    public class AMuleProxy : IAMuleProxy
    {
        ...
        private int GetOrCreateCategory(AMuleSettings settings)
        {
            ...
            var category = preferences.Categories.FirstOrDefault(v => string.Equals(v.Title, settings.MovieCategory, StringComparison.InvariantCultureIgnoreCase));
            ...
            categoryTag.Children.Add(AMuleEcTag.String(AMuleEcCodes.TagCategoryTitle, settings.MovieCategory));
            ...
            return GetPreferences(settings).Categories.First(v => string.Equals(v.Title, settings.MovieCategory, StringComparison.InvariantCultureIgnoreCase)).Id;
        }
        ...
        private sealed class AMuleConnection : IDisposable
        {
            ...
            private void Authenticate(AMuleSettings settings)
            {
                var authReq = new AMuleEcPacket(AMuleEcCodes.OpAuthReq)
                {
                    Tags =
                    {
                        AMuleEcTag.String(AMuleEcCodes.TagClientName, "Radarr"),
                        AMuleEcTag.String(AMuleEcCodes.TagClientVersion, "Radarr"),
