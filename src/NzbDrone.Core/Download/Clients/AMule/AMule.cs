...
namespace NzbDrone.Core.Download.Clients.AMule
{
    public class AMule : DownloadClientBase<AMuleSettings>
    {
        ...
        public override System.Threading.Tasks.Task<string> Download(RemoteMovie remoteMovie, IIndexer indexer)
        {
            if (!Ed2kLink.TryParse(remoteMovie.Release.DownloadUrl, out var link))
            {
        ...
        public override IEnumerable<DownloadClientItem> GetItems()
        {
            ...
            var category = Settings.MovieCategory.IsNullOrWhiteSpace()
                ? null
                : preferences.Categories.FirstOrDefault(v => string.Equals(v.Title, Settings.MovieCategory, StringComparison.InvariantCultureIgnoreCase));
            var outputDirectory = category?.Path.IsNotNullOrWhiteSpace() == true ? category.Path : preferences.IncomingDirectory;

            if (Settings.MovieCategory.IsNotNullOrWhiteSpace() && category == null)
            {
                yield break;
            }
        ...
        private ValidationFailure TestCategory(AMulePreferences preferences)
        {
            if (Settings.MovieCategory.IsNullOrWhiteSpace() ||
                preferences.Categories.Any(v => string.Equals(v.Title, Settings.MovieCategory, StringComparison.InvariantCultureIgnoreCase)))
            {
                return null;
            }
            ...
            {
                return new NzbDroneValidationFailure(nameof(Settings.MovieCategory), _localizationService.GetLocalizedString("DownloadClientAMuleValidationCategoryIncomingMissing"))
                {
                    DetailedDescription = _localizationService.GetLocalizedString("DownloadClientAMuleValidationCategoryIncomingMissingDetail")
                };
            }

            _logger.Debug("aMule category '{0}' does not exist and will be created when adding a download.", Settings.MovieCategory);
            return null;
        }
